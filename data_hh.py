...
class VacanciesJson(HHGetVacByAPI, VacanciesInFileJson, GetVacancies):
    ...
    def formatting_output_data(self, input_list):
        """Метод для преобразования json файла в удобный формат"""
        vacancies_list = []
        counter = 1
...
class HHUserInterface:
    ...
    def user_interaction():
        ...
        print(f"Вы выбрали платформу HeadHunter\n")
        city_name_input = input(f"Введите название города в котором ищете работу:").capitalize()
        prof_input = input(
            f"Введите название профессии:").capitalize()
        hh_instance = VacanciesJson(city_name_input, prof_input)
        hh_instance.save_to_json()
        print()
        if len(hh_instance.get_all_vac_info()) == 0:
            print(f"К сожалению по Вашему запросу вакансий нет. Попробуйте ввести по другому!")
        else:
            ...
            while True:
                ...
                function_input = input(f"Доступны следующие действия:\n"
                                       ...
                                       f"2 - Вывести топ вакансий (по убыванию)\n"
                ...
                # Вывод вакансий по ключевому слову в описании
                    ...
                    if len(key_word_filtered_list) > 0:
                        ...
                        print(f"К сожалению такой вакансии нет")
